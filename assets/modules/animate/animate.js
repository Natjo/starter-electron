const animate = (el, delay1) => {
    ...
    const duration =
        getComputedStyle(el).getPropertyValue("--duration") || ".5";
    const animation =
        getComputedStyle(el).getPropertyValue("--animation") || "fade";
        ...
        linify((line) => {
            [...line.trim()].forEach((val) => {
                if (val === " ") markup += "&nbsp;";
                    ...
                    const style = `animation:${animation} ${duration}s both ${
                        inc * delay + delay1
                    }s`;
    ...
    if (type == "list") {
        el.querySelectorAll("li").forEach((item, index) => {
            item.style.animation = `${animation} ${duration}s both ${
                index * delay + delay1
            }s`;
            el.classList.add("display");
