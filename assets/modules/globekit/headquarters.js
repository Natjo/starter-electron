import {
    ...
    CalloutManager,
    CalloutDefinition,
} from "/assets/modules/globekit/globekit.esm.js";
...
class MyGlobeKit {
    constructor(canvas, slug) {
        ...
        this.calloutManager = new CalloutManager(
            document.getElementById("callout-manager")
        );
            ...
            list.drawables.forEach((el) => {
                ...
                if (el.obj.id === this.points.id) {
                    if (el.selection !== undefined) {
                        ...
                        this.calloutManager.addCallout(
    ...
    addPoints(file) {
        ...
        this.points.transform = (element, point) => {
            point.size = 10;
            if (element.properties.type === "feed")
                point.color = [238 / 255, 119 / 255, 102 / 255, 1];
            else if (element.properties.type === "epc")
                point.color = [128 / 255, 199 / 255, 160 / 255, 1];
            else if (element.properties.type === "completed")
                point.color = [0 / 255, 112 / 255, 239 / 255, 1];
            return point;
        };
        fetch(`assets/json/${file}.json`)
    ...
    resize() {
        setTimeout(() => {
            ...
            //  this.gkview.movementModel.setLatLon(20, -111);
        }, 1);
    }
    open(file) {
        let inc = 0;
